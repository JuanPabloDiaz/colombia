...
const tailwindPathClasses =
  "fill-gray-300 stroke-gray-50 stroke-1 hover:fill-sky-500 cursor-pointer transition-colors duration-150 ease-in-out";

export default function InteractiveColombiaMap({ departments }) {
  // Added departments prop
  ...
  useEffect(() => {
    ...
    paths.forEach((path) => {
      ...
      const deptClass = originalClasses
        .split(" ")
        .find((cls) => cls.startsWith("sm_state_CO"));
      if (!deptClass) return;
      ...
      if (referenceName) {
        const department = departments.find(
          (d) =>
            d.name &&
            d.name.trim().toLowerCase() === referenceName.trim().toLowerCase(),
        );
        if (department) {
      ...
      newPath.addEventListener("mouseenter", (e) =>
        handleMouseEnter(e, svgDeptId, actualDeptName),
      );
    ...
    return () => {
      if (svgContainerRef.current) {
        ...
        if (currentSvgElement) {
          const currentPaths = currentSvgElement.querySelectorAll(
            "path[class*='sm_state_CO']",
          );
    ...
    <div className="relative h-auto w-full">
      ...
      {tooltipText && (
        <div
          ...
          className="bg-neutral text-neutral-content z-50 rounded p-2 text-sm shadow-lg"
        >
          {tooltipText}
        </div>
      )}
